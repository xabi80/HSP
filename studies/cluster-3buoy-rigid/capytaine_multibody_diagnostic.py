...
B4 multi-body BEM reader must parse.

Grid: the production single-body sweep, geomspace(0.1, 30, 80) rad/s
(matches studies/spar-fin-decay/capytaine_run.py). The earlier reduced
grid (geomspace(0.5, 8.0, 12)) resolved the heave cross-coupling that
Measurement B locked, but stopped at omega_max = 8 -- below the ~9.5-10
rad/s B-peak of surge/sway/roll/pitch -- so those DOFs sat at 100 %% of
peak at omega_max and produced truncation-ringing kernels that could not
pass the retardation Check-3 decay gate (M8 PR3 Step C finding). At
omega_max = 30 every DOF is past its peak (worst is yaw at ~6 %%), so the
full 18-DOF kernel decays and the coupled kernel gate runs end-to-end.
"""
...
_OMEGAS = np.geomspace(0.1, 30.0, 80)  # production grid; resolves all DOF B-peaks
...
def main() -> None:
    ...
    hb = np.array(
        [[float(Ainf.sel(radiating_dof=hi, influenced_dof=hj)) for hj in heave] for hi in heave]
    )
    ...
    print(
        f"  multi-body sum / composite - 1 = {rel:+.3%}  "
    ...
    print(
        f"  off-diagonal A33_ij: mean {offs.mean():+.4f}, "
        f"range [{offs.min():+.4f}, {offs.max():+.4f}]"
    )
    print(
        f"  |A33_ij| / A33_ii = {np.abs(offs).mean()/diag:.3%} "
