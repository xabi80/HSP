...
40 log-spaced omegas [0.1, 30] rad/s + inf; rho=1025, g=9.81; full
...
_OMEGA_MIN, _OMEGA_MAX, _N_OMEGA = 0.1, 30.0, 40
...
def _run_bem(mesh_path: Path, cog, out_nc: Path, label: str, omegas) -> None:
    ...
    problems = [
        cpt.RadiationProblem(body=body, omega=float(w), radiating_dof=d,
                             water_depth=float("inf"), rho=cc.RHO, g=cc.G)
        for w in omegas_rad for d in body.dofs
    ] + [
        cpt.DiffractionProblem(body=body, omega=float(w), wave_direction=0.0,
                               water_depth=float("inf"), rho=cc.RHO, g=cc.G)
        for w in omegas
    ]
    ...
    dataset["hydrostatic_stiffness"] = immersed.compute_hydrostatic_stiffness(
        rho=cc.RHO, g=cc.G
    )
...
def main() -> None:
    ...
    if mode == "reference":
        ...
        body = cpt.FloatingBody(mesh=m, center_of_mass=np.asarray(cog_comp),
                                name="probe")
        body.rotation_center = np.asarray(cog_comp)
        body.add_all_rigid_body_dofs()
        probe_omegas = [float(omegas[0]), float(omegas[len(omegas) // 2])]
        probes = [
            cpt.RadiationProblem(body=body, omega=w, radiating_dof=d,
                                 water_depth=float("inf"), rho=cc.RHO, g=cc.G)
            for w in probe_omegas for d in body.dofs
        ]
        t0 = time.perf_counter()
        ...
        print(f"  {len(probes)} probe problems in {dt:.1f} s "
              f"({per_problem:.2f} s/problem)")
        print(f"  full composite ~ {n_full} problems -> est {est:.0f} s "
              f"= {est/60:.1f} min")
        if est > 3 * 3600:
