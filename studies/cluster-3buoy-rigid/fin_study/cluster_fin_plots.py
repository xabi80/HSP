...
def row_at(d: Path, prefix: str, cfg: str, key: str, h: float):
    ...
    sub = sorted((r for r in rows if abs(float(r["height_m"]) - h) < 1e-9),
                 key=lambda r: float(r["period_s"]))
    return (np.array([float(r["period_s"]) for r in sub]),
            np.array([float(r[key]) for r in sub]))
...
def make_cluster_fig() -> None:
    ...
    fig.suptitle("3-buoy cluster (hub centre): fin-size sensitivity of heave RAO (top, "
                 "H=0.04 m) and Nz acceleration (bottom, H=0.12 m)\nrigorous coupled BEM per "
                 "fin; dotted = each fin's cluster heave natural period", fontsize=11)
...
def make_cross_model() -> None:
    ...
    for k, (_kind, key, ylab) in enumerate(
            [("rao", "rao_center", "peak heave RAO"),
             ("acc", "acc_center_amp", "peak Nz accel (m/s^2)")]):
        ...
        for xi, s, c in zip(x, sg, cl, strict=False):
            ax[k].annotate(f"{s:.2f}", (xi, s), fontsize=7, color="#1f77b4",
                           textcoords="offset points", xytext=(4, 4))
            ax[k].annotate(f"{c:.2f}", (xi, c), fontsize=7, color="#2ca02c",
    ...
    fig.suptitle("Fin-size sensitivity: single buoy vs 3-buoy cluster "
                 "(smaller fin -> right = higher motion, both models)", fontsize=12)
    fig.tight_layout(rect=(0, 0, 1, 0.95))
    fig.savefig(_CL / "fin_single_vs_cluster.png", dpi=150)
    plt.close(fig)
    print("wrote fin_single_vs_cluster.png")
    print("\npeak (Cd5) single / cluster:")
    for f in fins:
        cfg = f"{f}_Cd5" if f != "none" else "none_cap"
        print(f"  {_LBL[f]:22s}: RAO {_peak(_SG, '', cfg, 'rao_center'):.2f}/"
              f"{_peak(_CL, 'cluster_', cfg, 'rao_center'):.2f}  "
              f"accel {_peak(_SG, '', cfg, 'acc_center_amp'):.3f}/"
              f"{_peak(_CL, 'cluster_', cfg, 'acc_center_amp'):.3f}")
