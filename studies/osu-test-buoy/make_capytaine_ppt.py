...
"""
# ruff: noqa: RUF001  -- slide copy intentionally uses display typography (multiplication
...
def _panel(slide, l, t, w, h, header, sub, items, hc):
    ...
    body.fill.solid()
    body.fill.fore_color.rgb = RGBColor(0xF7, 0xFA, 0xFB)
    body.line.color.rgb = hc
    body.line.width = Pt(1.25)
    body.shadow.inherit = False
    hb = slide.shapes.add_shape(1, Inches(l), Inches(t), Inches(w), Inches(0.6))
    hb.fill.solid()
    hb.fill.fore_color.rgb = hc
    ...
    tb = slide.shapes.add_textbox(Inches(l + 0.28), Inches(t + 1.02),
                                  Inches(w - 0.52), Inches(h - 1.15))
    tf = tb.text_frame
    tf.word_wrap = True
    for i, (txt, opts) in enumerate(items):
        ...
        lead = p.add_run()
        lead.text = opts.get("mark", "•  ")
        lead.font.name = FONT
        lead.font.size = Pt(13)
        lead.font.color.rgb = hc
        r = p.add_run()
        r.text = txt
        r.font.name = FONT
        r.font.size = Pt(13)
...
def _cell(tbl, r, c, runs, fill, size=12, align=PP_ALIGN.LEFT):
    ...
    for txt, opts in runs:
        ...
        rr.font.bold = opts.get("b", False)
        rr.font.italic = opts.get("i", False)
...
def T(text, **kw):  # noqa: N802 -- terse text-run factory (~70 calls)
    return (text, kw)
...
p = tf.paragraphs[0]
r = p.add_run()
r.text = "DIFFRACTION & RADIATION ANALYSIS"
r.font.name = FONT
r.font.size = Pt(16)
r.font.bold = True
r.font.color.rgb = TEAL
...
r = p2.add_run()
r.text = "How we compute the buoy's\nhydrodynamics in Capytaine"
r.font.name = FONT
r.font.size = Pt(40)
r.font.bold = True
r.font.color.rgb = INK
...
p = tf.paragraphs[0]
r = p.add_run()
r.text = "OSU Test Buoy  ·  potential-flow BEM  →  time-domain FloatSim (Cummins)"
r.font.name = FONT
r.font.size = Pt(20)
r.font.bold = True
r.font.color.rgb = WHITE
p2 = tf.add_paragraph()
r = p2.add_run()
r.text = ("Two sub-problems (radiation + diffraction), their outputs, and how they "
...
_bullets(s, [
    (0, [T("Boundary-element (panel) solver for "),
     T("linear potential-flow hydrodynamics", b=True), T(".")]),
    (0, [T("In: the wetted hull as a mesh of panels.  Out: the flow (velocity potential φ) "
           "around it in regular waves at each frequency ω.")]),
    (0, [T("“Potential flow” = "),
         T("inviscid, irrotational, small (linear)", b=True, c=TEAL_D),
         T(" waves & motions — the one assumption that sets both the outputs and the limits.")]),
    (0, [T("Key idea — ", ), T("superposition", b=True), T(
        ": the wave-body problem splits into two sub-problems, solved separately and added:")]),
...
s.shapes.add_picture(str(HERE / "OSU_buoy_mesh_capytaine.png"),
                     Inches(3.35), Inches(1.5), height=Inches(4.98))
...
pic = s.shapes.add_picture(str(HERE / "Capytaine_explained.png"),
                           Inches(1.75), Inches(1.85), height=Inches(5.2))

# ---------------------------------------------------------- 4. Radiation
s = slide("Radiation — “body as a wavemaker”", "sub-problem ①")
_bullets(s, [
    (0, [T("Turn incident waves ", ), T("off", b=True), T(
        ". Force the body to oscillate in still water — one DOF at a time, at each ω.")]),
    (0, [T("It radiates its own waves; the reaction force has two pieces:")]),
    (1, [T("Added mass A(ω)", b=True, c=TEAL_D), T(
        " — in phase with acceleration (virtual mass of entrained water).")]),
    (1, [T("Radiation damping B(ω)", b=True, c=TEAL_D), T(
        " — in phase with velocity (energy carried off as radiated waves).")]),
...
_bullets(s, [
    ...
    (0, [T("Net wave-pressure force on the fixed hull = ", ),
     T("excitation force F_exc(ω, β)", b=True, c=TEAL_D), T("  (β = wave heading):")]),
    ...
    (0, [T("→ This run produces the wave forcing ", ), T(
        "F_exc(ω)", b=True), T(" that drives the motion.")]),
    (0, [T("Plus the hydrostatic stiffness ", ), T("C", b=True),
     T(" (buoyancy / waterplane) from the geometry.")]),
], t=2.0, gap=12)

# ---------------------------------------------------------- 6. Cummins bridge
s = slide("From frequency domain to time-domain simulation", "the bridge")
_bullets(s, [
    (0, [T("A(ω), B(ω) can’t go straight into a time-stepper — a transient contains all "
           "frequencies at once. Bridge = the "),
     T("Cummins equation", b=True), T(":")]),
], t=1.9, gap=6)
...
p.alignment = PP_ALIGN.CENTER
r = p.add_run()
r.text = "(M + A∞) ẍ  +  ∫₀ᵗ K(t−τ) ẋ(τ) dτ  +  C x  =  F_exc(t)  +  F_drag(ẋ)"
...
_bullets(s, [
    ...
    (0, [T("K(t) = (2/π)∫ B(ω) cos(ωt) dω", b=True, c=TEAL_D), T(" — the retardation kernel = ", ),
     T("fluid memory", i=True), T(" of previously radiated waves.")]),
    (0, [T("Mapping:  "), T("Radiation → A∞ + K(t)", b=True), T("   ·   "),
         T("Diffraction → F_exc", b=True), T("   ·   "), T("Hydrostatics → C", b=True),
         T(".  FloatSim integrates this in time.")]),
], t=4.1, gap=10)
...
arr = s.shapes.add_shape(MSO_SHAPE.RIGHT_ARROW, Inches(
    6.12), Inches(3.55), Inches(1.02), Inches(0.5))
arr.fill.solid()
arr.fill.fore_color.rgb = GREY
...
_bullets(s, [
    (0, [T("Potential flow is "), T("inviscid and linear", b=True),
         T(". Two consequences for us:")]),
    (1, [T("No viscous drag in the BEM", b=True, c=RED),
         T(" → all quadratic / viscous damping is added separately as "),
         T("Morison drag F_drag", b=True), T(" — not from Capytaine.")]),
    (1, [T("Panels treat every surface as solid", b=True, c=RED), T(" → "),
         T("over-predicts added mass", b=True),
         T(" for the open/perforated heave-plate frame (it blocks flow the real "
           "perforations pass).")]),
    (0, [T("Practical split for the OSU buoy:")]),
    (1, [T("Spar (6″ pipe)", b=True, c=TEAL_D), T(" — a clean cylinder → the BEM is reliable.")]),
    (1, [T("Heave-plate / ballast frame", b=True, c=TEAL_D), T(" — A and B are BEM ", ),
     T("stand-ins until the tank test measures them", b=True), T(".")]),
], t=2.0, gap=11)
...
data = [
    ...
    ([T("Water & BEM")], [T("fresh; deep; linear", b=True)],
     [T("OSU Hinsdale lab is fresh (ρ = 998); period is density-independent for a free body. "
        "Potential flow is inviscid — viscous drag added via Morison. Confirm tank depth "
        "vs 1.42 m draft.")]),
]
...
s.shapes.add_picture(str(HERE / "OSU_heave_decay_prediction.png"),
                     Inches(1.15), Inches(1.85), width=Inches(11.0))
...
_bullets(s, [
    ...
    (0, [T("Metrics: deck tilt (pitch), heave, and the moment carried at each "
           "buoy→platform joint.")]),
], t=2.1, gap=12)
