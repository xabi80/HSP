...
- **Accelerations at the sensor points** (platform centre + 4 cluster centres) are essentially
  unchanged with the walls in or out (`accel_wall_effect.png`, and the articulated version).
...
coupled_osu_walled.nc`, then `python articulated_wall.py decay` / `... rao`. The coupled `.nc`
files are large and regeneratable, so they are not committed. Caveats: coarse coupled mesh (the
