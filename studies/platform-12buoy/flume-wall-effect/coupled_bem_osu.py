"""Coupled 12-buoy OSU-plate BEM for the flume wall-effect articulated run.

OPEN or WALLED (method of images). 12 OSU-plate buoys (spar + equal-area heave-plate
disc) at the Phase-3 layout (buoy centres on a 2.5 m circle). Per-buoy rigid DOFs
(buoy{i}__Surge.. ) -> 72-DOF coupled radiation + diffraction, hydrostatic C tiled
from the single buoy. Saved in the FloatSim capytaine schema (read_capytaine-compatible).
...
def build_walled():  # type: ignore[no-untyped-def]
    """Combined mesh = 12 real + Y-images; custom dofs {label}__{DOF}_all (real+mirrored
    ...
    real_slices = [slice(starts[i], starts[i + 1]) for i in range(12)]
    npr = starts[12]
    img_slices = [[None, None] for _ in range(12)]
    off = npr
    ...
    body.dofs = dofs
    real_labels = [f"buoy{i + 1}__{d}" for i in range(12) for d in DOF6]
    return body, real_labels


def solve_freq(body, radiate_labels, read_labels, w, with_diff):  # type: ignore[no-untyped-def]
    solver = cpt.BEMSolver()
    probs = [cpt.RadiationProblem(body=body, omega=w, radiating_dof=r, water_depth=np.inf,
...
def main() -> None:
    ...
    A = np.zeros((len(om) + 1, 72, 72)); B = np.zeros((len(om) + 1, 72, 72))
    Fexc = np.zeros((len(om) + 1, 72), complex)   # last (omega=inf) row stays 0 (reader strips it)
    ...
    ds = xr.Dataset(
        data_vars=dict(
            ...
            hydrostatic_stiffness=(("radiating_dof", "influenced_dof"), C72),
        ...
        attrs=dict(rho=RHO, g=G, water_depth="inf", body_name=f"osu12_{mode}"),
    )
