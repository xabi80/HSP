"""Fin-sensitivity coupled 16-buoy platform BEM, MEMORY-BOUNDED. 16-buoy variant
of ``studies/platform-12buoy/platform_fin_bem.py``: the parametric spar+fin hull
(n_theta=16 coarse mesh) is replicated at the 16 platform positions
(``platform16_common.buoy_centers``) + platform draft -> 96-DOF coupled radiation
+ diffraction per fin {0.215, 0.15, none}.

Solves ONE frequency at a time with a FRESH BEMSolver each (per-frequency cache
freed between frequencies). Modes:
  test  -- 2 frequencies + inf; save, read back, verify the assembled NetCDF
  all / <tag>  -- full sweep per fin
"""
...
_NTHETA = 16
_COG_Z = cc.CoG_Z_SINGLE - pc16.PLATFORM_DZ
_FINS = {"0215": 0.215, "015": 0.15, "none": None}


def make_combined(r_fin: float | None):  # type: ignore[no-untyped-def]
    cfb._NTHETA = _NTHETA
    base = cfb.build_full_mesh(r_fin).translated([0.0, 0.0, -pc16.PLATFORM_DZ])
    bodies = []
    for i, (dx, dy) in enumerate(pc16.buoy_centers()):
...
def run(tag: str, r_fin: float | None, test: bool = False) -> None:
    ...
    out = _OUT / ("_test_platform16.nc" if test else f"capytaine_platform16_fin{tag}.nc")
    separate_complex_values(ds).to_netcdf(str(out))

    heave = [f"buoy{i + 1}__Heave" for i in range(pc16.N_BUOY)]
