...
def main() -> None:
    ...
    report_lines = [
        ...
        "=" * 70,
        f"Source mesh: studies/spar-fin-decay/mesh/test2_spar_fin.gdf",
        f"Original preserved at: studies/spar-fin-decay/mesh/test2_spar_fin_ORIGINAL.gdf",
        "",
        f"Detection criterion:",
        ...
        f"",
        f"Pre-fix normals (mean n_z):",
        f"  TOP: {nz[is_top].mean():+.4f} (expected +1.0; WRONG)",
        f"  BOT: {nz[is_bot].mean():+.4f} (expected -1.0; WRONG)",
        f"",
        f"Post-fix normals (mean n_z):",
        f"  TOP: {top_nz_after.mean():+.4f} (expected +1.0)",
        f"  BOT: {bot_nz_after.mean():+.4f} (expected -1.0)",
        f"",
        f"In-script assertions: ALL PASS",
        f"  TOP post-flip n_z > 0.5: PASS",
        f"  BOT post-flip n_z < -0.5: PASS",
        f"  Total flips in [200, 220]: PASS",
        f"",
        f"Next step: re-run capytaine_run.py and verify A_inf(heave) is",
        f"in the expected 25-50 kg range (vs the 1.30 kg from the",
        f"reversed-normal mesh).",
    ]
